...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatRetrieveDeliversFoundValuesOnNonEmptyCache(on sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        expect(sut, toCompleteWith: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatRetrieveHasNoSideEffectsOnNonEmptyCache(on sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        expect(sut, toCompleteTwiceWith: .success(CachedFeed(feed: feed, timestamp: timestamp)), file: file, line: line)
    }
    ...
    func assertThatInsertOverridesPreviouslyInsertedCacheValues(on sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        expect(sut, toCompleteTwiceWith: .success(CachedFeed(feed: lastestCache.feed, timestamp: lastestCache.timestamp)), file: file, line: line)
    }
    ...
    func insert(cache: (feed: [LocalFeedImage], timestamp: Date), into sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.insert(feed: cache.feed, timestamp: cache.timestamp) { receivedResult in
    ...
    func expect(_ sut: FeedStore, toCompleteTwiceWith expectedResult: FeedStore.RetreivalResult, file: StaticString = #filePath, line: UInt = #line) {
        expect(sut, toCompleteWith: expectedResult, file: file, line: line)
        expect(sut, toCompleteWith: expectedResult, file: file, line: line)
    }
    
    func expect(_ sut: FeedStore, toCompleteWith expectedResult: FeedStore.RetreivalResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retreive { retrieveResult in
            ...
            default:
                XCTFail("Expected to retreive \(expectedResult), but got \(retrieveResult) instead", file: file, line: line)
            }
